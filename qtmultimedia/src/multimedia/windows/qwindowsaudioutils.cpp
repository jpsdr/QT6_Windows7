...
{
    // Windows 7 backport: IAudioClient3 only exists from Windows 10 on, and activating it is
    // what used to fail here, leaving Windows 7 without any audio device at all. Ask for it
    // first so that nothing changes on Windows 10 and later - it is obtained exactly as before,
    // and the code below runs on the very same object. Only when it is unavailable do we fall
    // back to the base IAudioClient, which has been around since Vista and carries every method
    // the playback and capture paths actually use.
    ...
    // Setting the endpoint role goes through IAudioClient2::SetClientProperties, so it is only
    // available together with IAudioClient3 here. On Windows 10 and later this is reached exactly
    // as before; on Windows 7 the role simply stays at its default.
    if (role && audioClient3)
        audioClientSetRole(audioClient3, *role);
...
// Windows 7 backport: the three helpers below are reached during tear-down even when the
// audio client was never created, and they used to dereference the pointer unconditionally.
//
// createAudioClient() activates IAudioClient3, which only exists from Windows 10 on, so on
// Windows 7 activation fails, openAudioClient() returns false and m_audioClient stays empty -
// yet the stream object lives on, and QWASAPIAudioSinkStream::stop() still calls
// audioClientStop()/audioClientReset() on it. That crashed the application with an access
// violation as soon as playback was closed (the video itself plays, just without sound).
//
// Returning false matches what the callers already expect from a failed call.
bool audioClientStart(const ComPtr<IAudioClient> &client)
{
    if (!client)
        return false;
...
bool audioClientStop(const ComPtr<IAudioClient> &client)
{
    if (!client)
        return false;
...
bool audioClientReset(const ComPtr<IAudioClient> &client)
{
    if (!client)
        return false;
...
bool audioClientSetRate(const ComPtr<IAudioClient> &client, int rate)
{
